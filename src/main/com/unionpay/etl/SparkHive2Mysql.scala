      ...
      for(i <- 0 to interval){
        val results=sqlContext.sql(
          s"""
             ...
             | from (
             | select cup_branch_ins_id_nm,(case when cloud_pay_in='0' then 'apple pay'
             ...
             |    else '其它' end ) as cfp_sign ,to_date(settle_dt) as settle_dt,pri_acct_no
             | from hive_prize_discount_result
             | where  prod_in='0'  and  trans_id='S22'
             | ) tmp
             | group by tmp.cup_branch_ins_id_nm, tmp.cfp_sign, tmp.settle_dt
      """.stripMargin)
